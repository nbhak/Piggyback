...
// Type: vbAccelerator.Components.Shell.ShellLink
  ...
  public class ShellLink : IDisposable
  {
    ...
    public void Dispose()
    {
      if (this.linkW != null)
      {
        ...
        this.linkW = (ShellLink.IShellLinkW) null;
      }
      if (this.linkA == null)
        return;
      Marshal.ReleaseComObject((object) this.linkA);
      this.linkA = (ShellLink.IShellLinkA) null;
    }
    ...
    public void Open(string linkFile) => this.Open(linkFile, IntPtr.Zero, ShellLink.EShellLinkResolveFlags.SLR_ANY_MATCH | ShellLink.EShellLinkResolveFlags.SLR_NO_UI, (ushort) 1);

    public void Open(string linkFile, IntPtr hWnd, ShellLink.EShellLinkResolveFlags resolveFlags) => this.Open(linkFile, hWnd, resolveFlags, (ushort) 1);

    public void Open(
      string linkFile,
      IntPtr hWnd,
      ShellLink.EShellLinkResolveFlags resolveFlags,
      ushort timeOut)
    {
      uint fFlags = (resolveFlags & ShellLink.EShellLinkResolveFlags.SLR_NO_UI) != ShellLink.EShellLinkResolveFlags.SLR_NO_UI ? (uint) resolveFlags : (uint) (resolveFlags | (ShellLink.EShellLinkResolveFlags) ((int) timeOut << 16));
      if (this.linkA == null)
      {
    ...
    private interface IShellLinkA
    {
      void GetPath(
        ...
        int cchMaxPath,
        ref ShellLink._WIN32_FIND_DATAA pfd,
        uint fFlags);
    ...
    private interface IShellLinkW
    {
      void GetPath(
        ...
        int cchMaxPath,
        ref ShellLink._WIN32_FIND_DATAW pfd,
        uint fFlags);
